...
function (_React$Component) {
  ...
  _proto.render = function render() {
    var _this$props = this.props,
        ...
        props = (0, _objectWithoutPropertiesLoose2.default)(_this$props, ["date"]); // console.log(date, props, 'date n props')
...
Week.range = function (date, _ref) {
  ...
  var firstOfWeek = localizer.startOfWeek(); // let start = dates.startOf(date, 'week', firstOfWeek)
  // let end = dates.endOf(date, 'week', firstOfWeek)

  var start = date;

  var end = _dates.default.add(start, 6, 'day'); // console.log(start, 'start only')
...
Week.title = function (date, _ref2) {
  ...
  var _Week$range = Week.range(date, {
    localizer: localizer
      ...
      start = _Week$range[0],
      rest = _Week$range.slice(1); // console.log(start, ...rest, 'rest')
